...
const blocks = {
  ...
  header_one: props => <h1 {...props.attributes}>{props.children}</h1>
}
...
class SlateMarkdownEditor extends React.Component {
  ...
  onBackspace = (event, change) => {
    ...
    if (startBlock.type == 'paragraph' && depth <= 1) return
  ...
  onKeyDown = (event, change) => {
    if(event.key == ' ') {
      ...
      if(
        /.*[^\w\s]+\s\w/.test(startBlock.text + event.key)
      ) {  
        let fragment = deserialize(startBlock.text + event.key)
       
        event.preventDefault()
        let parentNode = this.parent(change.value)
        let node = fragment.nodes.first()
